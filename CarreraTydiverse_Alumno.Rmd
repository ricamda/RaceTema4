...
```{r}
avg_bytes <- cran%>%summarise(mean(size));avg_bytes
```
# Agrupamientos
...
```{r}
library(tidyr)
by_package <- group_by(cran, package);by_package
ungroup(by_package)
...
top_counts_sorted <- cran %>% count(package)%>%
  filter(n > quantile(n, probs = 0.99))%>% arrange(desc(n))
